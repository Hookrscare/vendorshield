...
import { CloudInspectionPanel } from "@/components/snapinspect/CloudInspectionPanel";
...
export default function SnapInspectAppPage() {
  ...
  const saveInspections = (newInspections: InspectionData[]) => {
    ...
    try {
      ...
      setLocalSaveError("");
    } catch { setLocalSaveError("Device storage is full. Changes are in memory only; export a backup or save to your workspace now."); }
  };
  ...
  const handleCreateNewInspection = () => {
    const newId = `insp-${crypto.randomUUID()}`;
    const newInspection: InspectionData = {
      ...
      executiveSummary: "Inspection findings have not yet been entered. Review all fields before sharing.",
      defects: [],
      status: "draft",
  ...
  return (
    <div className="bg-gray-950 text-gray-100 min-h-screen pb-20">
      <div className="mx-auto max-w-7xl p-4 text-sm text-amber-200 bg-amber-500/10">Starts with sample records. Edit locally, then choose Save to workspace for a cloud copy. Share only reports you have reviewed. Voice suggestions use keyword rules and require review.</div>
      {localSaveError && <p role="alert" className="p-4 text-rose-200">{localSaveError}</p>}
      <CloudInspectionPanel inspection={activeInspection} onLoad={(cloudInspection) => {
        const existing = inspections.find(item => item.id === cloudInspection.id);
        const preserved = existing && JSON.stringify(existing) !== JSON.stringify(cloudInspection)
          ? [{ ...existing, id: `local-${crypto.randomUUID()}`, title: `${existing.title} (local backup)`.slice(0, 240) }] : [];
        saveInspections([cloudInspection, ...preserved, ...inspections.filter(item => item.id !== cloudInspection.id)]);
        setActiveInspectionId(cloudInspection.id);
      }} />
            ...
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-medium border bg-gray-950/70 shrink-0">
              {syncState.isOnline ? (
                <>
                  ...
                  <span className="text-emerald-400 font-semibold">Online · draft stored locally</span>
                </>
              ) : (
                <>
        ...
        {activeTab === "preview" && (
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-red-950/40 via-gray-900 to-gray-900 border-2 border-red-500/40 rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-2xl">
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto">
                <button
                  disabled title="Use Workspace storage & client report above to create a reviewed snapshot link."
                ...
                >
                  ...
                  <span>Share via workspace panel</span>
                </button>
