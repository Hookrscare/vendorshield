...
| SnapInspect `/snapinspect`, `/snapinspect/app` | Local prototype: records, photos, browser speech when supported, keyword parser, PDF drafts | `src/app/snapinspect/app/page.tsx`, `src/components/snapinspect/PhotoUpload.tsx`, `VoiceRecorder.tsx`, `src/lib/snapinspect/voice-parser.ts` and PDF exporter. Starts with samples. Browser/device speech and offline coverage remain unverified; no claim of production field readiness. |
| SnapInspect cloud sync / client link | Unavailable | `offline-sync.ts` previously cleared pending queue without a server acknowledgement; now retains it and returns zero synced. Client link only added `?report=` with no shared data retrieval; disabled. Existing local inspections preserved. |
