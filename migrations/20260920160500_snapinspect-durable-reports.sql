-- Durable immutable document snapshots live in a private Storage bucket.
-- Postgres holds only bounded metadata and revision/share pointers.
alter table public.inspections
  ...
  add column share_document_key text,
  add column share_expires_at timestamptz;
create unique index inspections_org_client_id on public.inspections(organization_id, client_id);
create unique index inspections_share_hash on public.inspections(share_token_hash) where share_token_hash is not null;
revoke all on public.inspections from anon, authenticated;
grant select on public.inspections to authenticated;
-- Existing member SELECT policy remains; all mutations go through checked RPCs.

create function public.save_inspection_document(target_org uuid, target_client_id text, expected_revision integer, new_title text, new_document_key text, new_document_url text)
...
begin
  ...
  if target_client_id !~ '^[a-zA-Z0-9_-]{1,100}$' or char_length(new_title) not between 1 and 240 or expected_revision < 0 then raise exception 'invalid_inspection'; end if;
  ...
  if found then
    if record.revision <> expected_revision then raise exception 'revision_conflict' using errcode='40001'; end if;
...
create function public.set_inspection_share(target_org uuid, target_client_id text, expected_revision integer, token_hash text)
returns jsonb language plpgsql security definer set search_path = '' as $$
declare record public.inspections; caller_role text;
begin
  select role into caller_role from public.organization_members where organization_id=target_org and user_id=auth.uid();
